...
ticker_comparison = st.text_input(f"Enter Stock Symbol to Compare against {primary_ticker}:", value="AAPL")
...
for ticker in user_list:
    ...
    while not success and attempts < max_retries:
        try:
            ...
            attempts += 1
            #st.write(f"Attempt {attempts} for {ticker} failed: {e}")
...
monthly_comparison_filtered = monthly_comparison[monthly_comparison['Ticker'] == primary_ticker]
...
monthly_comparison_resampled = monthly_comparison_filtered.resample('M').agg(custom_aggregation)
...
# Select only the Date, Ticker, and Close columns and set Date as index
if not all_data.empty:
    all_data = all_data.reset_index()[['Date', 'Ticker', 'Close']].set_index('Date')
